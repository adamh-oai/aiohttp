...
class ClientEngineConformanceTests:
    """Reusable behavior checks for alternate client engine implementations."""

    def make_session(self, **kwargs: object) -> ClientSession:
    ...
    async def test_request_raise_for_status_can_disable_session_policy(
        ...
        async with self.make_session(
            ...
            response = await session.get(server.make_url("/"), raise_for_status=False)
    ...
    async def test_streaming_response_can_be_consumed_before_eof(
        ...
        async with self.make_session() as session:
            response = await asyncio.wait_for(
                session.get(server.make_url("/")), timeout=1
            )
...
class TestAsyncioClientEngineConformance(ClientEngineConformanceTests):
    def make_session(self, **kwargs: object) -> ClientSession:
